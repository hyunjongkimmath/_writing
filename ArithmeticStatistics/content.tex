...
\subsection{Batryev-Manin conjecture}

\begin{conjecture}[Batyrev-Manin Conjecture] \label{conjecture:batyrev_manin_conjecture}
    \TODO{verify these formulations}
Let $K$ be a \CrefAndHyperrefIfExist{definition:number_field}{number field} and let $X$ be a smooth \CrefAndHyperrefIfExist{definition:projective_variety}{projective variety} over $K$ such that the anticanonical bundle $\omega_X^{-1}$ is \CrefAndHyperrefIfExist{definition:ample_divisor_on_a_scheme}{ample} (i.e., $X$ is a \CrefAndHyperrefIfExist{definition:fano_variety_over_a_field}{Fano variety}). 

\TODO{height function}
Let $H$ be a height function on $X(K)$ associated to $\omega_X^{-1}$. Then there exists a non-empty open subvariety $U \subseteq X$ such that the counting function 
$$N_U(B) = \#\{P \in U(K) : H(P) \leq B\}$$
satisfies the following asymptotic growth as $B \to \infty$:
$$N_U(B) \sim c \cdot B \cdot (\log B)^{\rho(X) - 1},$$
where:
\begin{enumerate}
    \item $c$ is a positive constant (often called the \hldef{Peyre constant}).
    \item $\rho(X)$ is the rank of the \CrefAndHyperrefIfExist{definition:picard_group}{Picard group} $\operatorname{Pic}(X)$.
\end{enumerate}

More generally, for an arbitrary ample line bundle $L$, the conjecture predicts:
$$N_U(L, B) \sim c B^{a(X, L)} (\log B)^{b(X, L) - 1},$$
where $a(X, L)$ is the \hldef{Fujita invariant} and $b(X, L)$ is related to the rank of the Picard group.
\end{conjecture}
