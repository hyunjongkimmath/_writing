...
$$\calC^{\op} \times \calD \to \Sets$$
given by 
\begin{align*}
(X,Y) &\mapsto \operatorname{Hom}_{\mathcal{D}}(F(X), Y) \\
(X,Y) &\mapsto \operatorname{Hom}_{\mathcal{C}}(X, G(Y))
\end{align*}
respectively, and there is a natural isomorphism between them.

In this case, the \hldef{unit of the adjunction} is the natural transformation $\eta : \mathrm{Id}_{\mathcal{C}} \Rightarrow G F$ such that, 
\begin{enumerate}
