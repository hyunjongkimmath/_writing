...
Given a \CrefAndHyperrefIfExist{definition:grothendieck_universe}{universe} $U$, we can define the notion of a \hldef{$U$-locally small category} and of a \hldef{$U$-small category} similarly \cite[Expos\'e I, D\'efinition 1.1.]{SGA4_I}. More explicitly, 
\begin{enumerate}
    ...
    \item a $U$-small category is a category such that $\operatorname{Ob}(\mathcal{C})$ is a $U$-set and for every pair of objects $X, Y \in \operatorname{Ob}(\mathcal{C})$, the collection $\operatorname{Hom}_{\mathcal{C}}(X,Y)$ of morphisms between them is a $U$-set; in particular the collection of all objects and morphisms in a $U$-small category is a $U$-set.
\end{enumerate}
A \hldef{$U$-category} refers to a $U$-locally small category.
}
\end{definition}
