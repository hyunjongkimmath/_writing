...
% Let $M$ be a topological manifold of dimension $n$. A \hldef{$C^k$-atlas} (or \hldef{smooth atlas} if $k = \infty$) on $M$ is an \CrefAndHyperrefIfExist{definition:atlas_on_a_topological_manifold}{atlas}
% $$ \mathcal{A} = \{(U_\alpha, \varphi_\alpha)\}_{\alpha \in A} $$
% such that for every pair $\alpha, \beta \in A$ with $U_\alpha \cap U_\beta \neq \emptyset$, the \CrefAndHyperrefIfExist{definition:transition_map_between_charts_on_a_topological_manifold}{transition maps}
% $$ \varphi_\beta \circ \varphi_\alpha^{-1} : \varphi_\alpha(U_\alpha \cap U_\beta) \to \varphi_\beta(U_\alpha \cap U_\beta) $$
% are \CrefAndHyperrefIfExist{definition:C_k_map_between_open_subsets_of_closed_half_spaces_of_Rns}{$C^k$-diffeomorphisms} between open subsets of $\mathbb{R}^n$.
