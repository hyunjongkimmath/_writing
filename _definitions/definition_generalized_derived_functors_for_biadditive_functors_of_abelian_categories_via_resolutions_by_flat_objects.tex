\begin{definition}[Derived functors for biadditive functors of abelian categories via resolutions by flat objects] \label{definition:generalized_derived_functors_for_biadditive_functors_of_abelian_categories_via_resolutions_by_flat_objects}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            \item Let \hl{$L_n^{II}F(A, B) = L_n^{II}(F(A, -))(B)$} be an object of $\calC$ obtained as follows: take a \CrefAndHyperrefIfExist{definition:left_right_resolution_of_an_object_by_a_class_of_objects_in_an_abelian_category}{left resolution} 
            $$P_\bullet \to B$$
            ...
            (\Cref{definition:homology_and_cohomology_objects_for_a_chain_complex_in_an_abelian_category}).

            \item Let \hl{$R_{II}^{n}F(A, B) = R_{II}^{n}(F(A, -))(B)$} be an object of $\calC$ obtained as follows: take a \CrefAndHyperrefIfExist{definition:left_right_resolution_of_an_object_by_a_class_of_objects_in_an_abelian_category}{right resolution} 
            $$B \to I^\bullet$$
        ...
        \begin{enumerate}
            \item Let \hl{$L_n^{I}F(A, B) = L_n^{I}(F(-, B))(A)$} be an object of $\calC$ obtained as follows: take a \CrefAndHyperrefIfExist{definition:left_right_resolution_of_an_object_by_a_class_of_objects_in_an_abelian_category}{left resolution} 
            $$P_\bullet \to A$$
            ...
            \item Let \hl{$R_{I}^{n}F(A, B) = R_{I}^{n}(F(-, B))(A)$} be an object of $\calC$ obtained as follows: take a \CrefAndHyperrefIfExist{definition:left_right_resolution_of_an_object_by_a_class_of_objects_in_an_abelian_category}{right resolution} 
            $$A \to I^\bullet$$
    ...
    In case that $F$ is a functor thought of as some kind of ``Hom'' and denoted as (some variation of) $\Hom: \calA^{\op} \times \calB \to \calC$ (most usually, $\calA = \calB$ for such Hom functors), it is customary to denote $R_n^{II} F(A,B)$ and $R_n^{I} F(A,B)$ by \hl{$\mathrm{Ext}_n^{II}(A,B)$} and \hl{$\mathrm{Ext}_n^{I}(A,B)$} respectively and to call these objects \hldef{Ext objects}. 
\end{definition}