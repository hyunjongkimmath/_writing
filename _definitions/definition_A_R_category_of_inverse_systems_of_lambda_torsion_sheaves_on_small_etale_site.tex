...
It is worth noting that $\mathrm{AR}(\lambda\text{-tors}(X))$ is an \hyperrefIfExists{definition:abelian_category}{abelian category}\CrefIfExists{definition:abelian_category} and that $\calF \in \mathrm{AR}(\lambda\text{-tors}(X))$ is zero if and only if it is a \CrefAndHyperrefIfExist{definition:null_system_in_a_pointed_category_with_images}{null system}.
\end{definition}
