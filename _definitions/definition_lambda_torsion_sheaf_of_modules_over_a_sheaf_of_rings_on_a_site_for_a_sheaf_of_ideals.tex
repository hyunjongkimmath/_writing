...
% Let $R$ be a commutative ring, $\lambda\subseteq R$ an ideal, and $X$ a scheme. A sheaf $\mathcal{F}$ of $R$-modules on the \CrefAndHyperrefIfExist{definition:small_etale_site_of_a_scheme}{(small) étale site $X_{\text{ét}}$}\CrefIfExists{definition:small_etale_site_of_a_scheme} is called a \hldef{$\lambda$-torsion sheaf} if for every \CrefAndHyperrefIfExist{definition:etale_morphism_of_schemes}{étale morphism} $U\to X$, every section $s\in \mathcal{F}(U)$ is annihilated by some power of $\lambda$, i.e.
% \[
