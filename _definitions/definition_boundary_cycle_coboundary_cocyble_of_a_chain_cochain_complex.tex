\begin{definition} \label{definition:boundary_cycle_coboundary_cocyble_of_a_chain_cochain_complex}
    ...
    \begin{enumerate}
        \item Let $C_\bullet = (\dots \to C_{n+1} \xrightarrow{d_{n+1}} C_n \xrightarrow{d_n} C_{n-1} \to \dots)$ be a \CrefAndHyperrefIfExist{definition:chain_complex_of_objects_in_a_preadditive_category}{chain complex} in $\mathcal{A}$.
        For each integer $n$, we define:
        \begin{itemize}
            \item The object of \hldef{$n$-cycles}, denoted \hl{$Z_n(C)$}, is the \CrefAndHyperrefIfExist{definition:kernel_and_cokernel_of_a_morphism_in_a_pointed_category}{kernel} of the differential leaving $C_n$:
        ...
        For each integer $n$, we define:
        \begin{itemize}
            \item The object of \hldef{$n$-cocycles}, denoted \hl{$Z^n(C)$}, is the \CrefAndHyperrefIfExist{definition:kernel_and_cokernel_of_a_morphism_in_a_pointed_category}{kernel} of the differential leaving $C^n$:
