...
\CrefAndHyperrefIfExist{definition:chain_complex_of_objects_in_a_preadditive_category}{chain complexes} in~$\mathcal{A}$, whenever the corresponding sums or products exist. 
The complexes $\Tot_{\oplus}^\bullet(A)$ and $\Tot_{\Pi}^\bullet(A)$ may also be denoted by notations such as \hl{$\Tot^{\oplus}(A)$} and \hl{$\Tot^{\Pi}(A)$} --- note the superscripted direct sum and product symbols --- especially when \CrefAndHyperrefIfExist{definition:chain_complex_of_objects_in_a_preadditive_category}{cohomological conventions} are adopted.
\end{definition}