...
An \hl{algebraic group scheme over $S$} (or an \hl{$S$-group scheme}) is a group object $G$ in the category of schemes over $S$; that is, $G$ is an $S$-scheme equipped with $S$-morphisms:
\hl{$m: G \times_S G \to G$} (\hldef{multiplication}), \hl{$i: G \to G$} (\hldef{inverse}), and \hl{$e: S \to G$} (\hldef{identity}/\hldef{identity section}),
...
If the multiplication map also satisfies the commutative axiom, then the group scheme $G$ is called \hldef{commutative} or \hldef{abelian}.
