...
\begin{definition}[Localization by a multiplicative system] \label{definition:localization_of_a_category_by_a_multiplicative_system}
    Given a category $\mathcal{C}$ and a \CrefAndHyperrefIfExist{definition:multiplicative_system_of_morphisms_in_a_category}{multiplicative system} $S \subseteq \mathrm{Mor}(\mathcal{C})$, the \hldef{localization} \hl{$\mathcal{C}[S^{-1}]$} is a category equipped with a functor
