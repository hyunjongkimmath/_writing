\begin{definition} \label{definition:fourier_transform_of_an_L_1_function_on_R}
    ...
    If $f \in L^1(\bbR)$, then the integrand $x \mapsto f(x)e^{-ixt}$ is a \CrefAndHyperrefIfExist{definition:vector_valued_lebesgue_integral_into_normed_vector_space}{Bochner integrable function} with values in $\bbC$, and these integrals are all well-defined.
\end{definition}