...
The continuity of $u$ implies that $u^s\mathcal{F}$ is indeed a sheaf on $(\calC,J)$\CrefIfExists{proposition:equivalent_conditions_for_continuous_functor_of_sites}. The assignment $\mathcal{F} \mapsto u^s\mathcal{F}$ defines the \hldef{direct image/pushforward functor}
...
Note that the ``\CrefAndHyperrefIfExist{definition:inverse_image_of_sheaves_along_a_cocontinuous_functor_of_sites}{inverse image of a cocontinuous functor of sites}'' and the ``direct image of a continuous functor of sites'' share the same constructions, i.e. by precomposition. More precisely, if $u: \calC \to \calD$ is a \CrefAndHyperrefIfExist{definition:continuous_functor_of_sites}{continuous} and cocontinuous functor of sites, then the direct image $u_*$ of $u$ as a continuous functor and the \CrefAndHyperrefIfExist{definition:inverse_image_of_sheaves_along_a_cocontinuous_functor_of_sites}{inverse image $u^*$ of $u$ as a cocontinuous functor} are exactly the same.
