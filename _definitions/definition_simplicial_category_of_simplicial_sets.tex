\begin{definition} \label{definition:simplicial_category_of_simplicial_sets}
    ...
    (i.e. $\mathbf{s}\Sets$ can be regarded as a category \CrefAndHyperrefIfExist{definition:category_enriched_in_a_monoidal_category}{enriched} in $\mathbf{s}\Sets$) as follows: given simplicial sets $X$ and $Y$, we let \hl{$\operatorname{Mor}(X,Y)$}, which is also often denoted by notations such as \hl{$Y^X$}, \hl{$\Map(X,Y)$}, or \hl{$\Fun(X,Y)$}, and referred to as the \hldef{exponential simplicial set from $X$ to $Y$}, the \hldef{simplicial mapping object from $X$ to $Y$}, or the \hldef{mapping simplicial set from $X$ to $Y$}
