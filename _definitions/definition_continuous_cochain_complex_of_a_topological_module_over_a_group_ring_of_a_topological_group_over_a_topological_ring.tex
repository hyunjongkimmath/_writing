...
Let $M$ be an object in $R\text{-}\mathbf{TopMod}_G$. The \hldef{continuous cochain complex} \hl{$C^\bullet_{\text{cont}}(G, M)$} (also denoted by \hl{$C^\bullet(G,M)$}, but which should not be confused with the cochain complex \CrefAndHyperrefIfExist{definition:group_cochain_complex_with_coefficients_in_a_group_module_over_a_ring}{$C^\bullet(G,M)$} that does not incorporate any topological structure) is defined as follows:
\begin{enumerate}
