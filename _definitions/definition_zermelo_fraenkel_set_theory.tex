...
\begin{enumerate}
  ...
  \item \textbf{Axiom of Empty Set:} There exists a set with no elements.
  $$\exists S\, \forall x\, (x \notin S).$$
  By extensionality, this set is unique; it is denoted by \hl{$\varnothing$}.
  ...
  One commonly writes $B$ in the following manners:
  $$\hlin{\{x \in A: \varphi(x)\}}$$
  or
  $$\hlin{\{x \in A \mid \varphi(x)\}.}$$

  \TODO{definable function}
  \item \textbf{Axiom Schema of Replacement:} For any definable function given by a formula $\varphi(x,y)$ ensuring that for each $x$ there exists a unique $y$ with $\varphi(x,y)$, the \CrefAndHyperrefIfExist{definition:function_of_sets}{image} of any set under this function is also a set.
