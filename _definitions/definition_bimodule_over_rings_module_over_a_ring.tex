\begin{definition} \label{definition:bimodule_over_rings_module_over_a_ring}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        making $M$ a \CrefAndHyperrefIfExist{definition:bimodule_over_rings_module_over_a_ring}{left $R$-module},
    ...
    % an abelian group $(M,+)$ which is simultaneously a left $R$-module and a right $R$-module, such that $(rm)s = r(ms)$ for all $r,s \in R$, $m \in M$. Equivalently, a two-sided $R$-module is an \hldef{$R$-$R$-bimodule}\CrefIfExists{definition:bimodule_over_rings_module_over_a_ring}
...
Any abelian group is equivalent to a two-sided $\bbZ$-module\CrefIfExists{proposition:category_of_abelian_groups_is_equivalent_to_categories_of_Z_modules}. Moreover, any left $R$-module is equivalent to an \CrefAndHyperrefIfExist{definition:bimodule_over_rings_module_over_a_ring}{$R-\bbZ$-bimodule}\CrefIfExists{proposition:category_of_left_R_modules_is_equivalent_to_R_Z_bimodules} and any right $R$-module is equivalent to an \CrefAndHyperrefIfExist{definition:bimodule_over_rings_module_over_a_ring}{$\bbZ-R$-bimodule}\CrefIfExists{proposition:category_of_right_R_modules_is_equivalent_to_Z_R_bimodules}. Given a left/right/two-sided $R$-module, its \hldef{natural bimodule structure} will refer to its structure as a $R$-$\bbZ$/$\bbZ$-$R$/$R$-$R$ bimodule. In this way, many definitions associated with the notions of left/right/two-sided $R$-modules can be defined as special cases for definitions for $R$-$S$-bimodules.
\end{definition}
