...
We write such a natural transformation by \hl{$\eta: F \Rightarrow G$}. We might also write \hl{$\eta: F \Rightarrow G: \calC \to \calD$} to fully specify that $\eta$ is a natural transformation from $F$ to $G$, which are in turn functors from $\calC$ to $\calD$.  
