...
Let $k \in \mathbb{Z}_{\geq 0}$ and let $\chi$ be a \CrefAndHyperrefIfExist{definition:dirichlet_character_modulo_a_nonnegative_integer}{Dirichlet character modulo $k$}. For $s \in \mathbb{C}$, the \hldef{$L$-function/series of $\chi$} is defined as
...
\CrefAndHyperrefIfExist{theorem:analytic_continuation_and_functional_equation_for_dirichlet_L_function}{It extends meromorphically to $\bbC$ with a single simple pole at $s = 1$ when $\chi$ is principal and is entire otherwise, and satisfies a functional equation.}
