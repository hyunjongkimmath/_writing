...
\begin{definition} \label{definition:absolute_frobenius_morphism_on_a_scheme}
    ...
    \begin{enumerate}
        \item The \hldef{$p^n$th power absolute Frobenius morphism on $X$} is the scheme endomorphism $X \to X$, denoted commonly by notations such as \hl{$F_{X,p^n}$} and \hl{$\Frob_{X,p^n}$} unless otherwise specified, given by the identity on the underlying topological space and affine locally by morphisms $\Spec A \to \Spec A$ whose corresponding ring morphism $A \to A$ is the \CrefAndHyperrefIfExist{definition:absolute_frobenius_endomorphism_of_a_ring}{$p^n$th power absolute Frobenius endomorphism}. 
