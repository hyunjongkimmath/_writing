...
\Cref{theorem:colimits_commute_with_colimits_and_limits_commute_with_limits} states that colimits commutes with colimits and limits commute with limits.
...
\input{../_concepts/lemma_equivalence_of_functor_categories_under_equivalent_source_categories.tex}
...
\input{../_concepts/lemma_hom_to_from_zero_object_in_preadditive_category_is_zero_group.tex}
...
\begin{example}
    \begin{enumerate}
        \item A \CrefAndHyperrefIfExist{definition:category_enriched_in_a_monoidal_category}{category enriched in} $\Sets$ is equivalent to a \CrefAndHyperrefIfExist{definition:locally_small_category}{locally small category}. 
        \item A category enriched in $\Ab$ is equivalent to a \CrefAndHyperrefIfExist{definition:additive_category_preadditive_category}{pre-additive category}. 
...
\input{../_concepts/lemma_sieve_containing_identity_morphism_is_maximal_sieve_on_the_object.tex}
...
\input{../_concepts/proposition_sieves_as_subfunctors_of_representable_functors.tex}
...
\input{../_definitions/definition_cech_nerve_of_a_family_of_morphisms_to_an_object_in_a_category.tex}

\input{../_definitions/definition_sheaf_on_a_grothendieck_pretopology_valued_in_a_category.tex}
...
\input{../_concepts/proposition_sieve_containing_covering_sieve_is_covering_with_respect_to_grothendieck_topology.tex}
...
\input{../_concepts/theorem_grothendieck_topology_on_a_cateogry_generated_by_a_pretopology_is_indeed_a_grothendieck_topology_and_is_the_coarsest_one_containing_all_the_covering_sieves_generated_by_the_pretopology_families.tex}
...
\input{../_concepts/theorem_limits_of_presheaf_diagrams_valued_in_a_category_are_computed_sectionwise.tex}
\input{../_concepts/theorem_limits_of_sheaf_diagrams_on_a_site_valued_in_a_category_are_computed_sectionwise.tex}

\input{../_concepts/proposition_sheaf_on_pretopology_equivalent_to_sheaf_on_generated_topology.tex}
...
\input{../_definitions/definition_morphism_between_bimodules_of_different_rings_over_a_morphism_of_the_rings.tex}
\input{../_definitions/definition_category_of_all_bimodules_over_all_rings.tex}

\input{../_definitions/definition_category_of_tensor_composable_bimodules_over_all_rings.tex}

\input{../_definitions/definition_tensor_product_functor_on_the_category_of_tensor_composable_bimodules_over_all_rings.tex}

\begin{proposition}
The tensor product functor is associative up to a natural isomorphism. For any three composable bimodules ${}_R M_S$, ${}_S N_T$, and ${}_T P_U$, there exists a natural isomorphism in $\mathcal{B}imod$:
$$\alpha_{M,N,P}: (M \otimes_S N) \otimes_T P \xrightarrow{\cong} M \otimes_S (N \otimes_T P)$$
defined by the mapping $((m \otimes n) \otimes p) \mapsto (m \otimes (n \otimes p))$. This data, along with the unit bimodules ${}_R R_R$, endows $\mathcal{B}imod$ with the structure of a \CrefAndHyperrefIfExist{definition:bicategory}{bicategory}.
\end{proposition}


\input{../_definitions/definition_category_of_hom_composable_bimodules_over_all_rings.tex}
...
\subsubsection{Continuous functors between sites and Morphisms between sites}
\input{../_definitions/definition_continuous_functor_of_sites.tex}
...
\input{../_definitions/definition_equivalence_of_sites.tex}

\input{../_concepts/lemma_equivalence_of_sheaf_categories_under_equivalent_sites.tex}

\input{../_concepts/proposition_unique_site_structure_induced_by_equivalence_of_categories.tex}
...
\subsubsection{cocontinuous functors between sites}

\input{../_definitions/definition_cocontinuous_functor_of_sites.tex}
\input{../_concepts/proposition_functor_between_sites_is_cocontinuous_if_and_only_if_precomposition_functor_sends_sheaves_to_sheaves.tex}
\input{../_definitions/definition_left_kan_extension_of_presheaves_along_a_functor_of_sheaves_along_cocontinuous_functor_of_sites.tex}
\input{../_definitions/definition_inverse_image_of_sheaves_along_a_cocontinuous_functor_of_sites.tex}

\subsubsection{Morphisms of sites and of topoi}

There are multiple types of morphisms of topoi induced by functors between sites, depending on whether said functors are continuous or cocontinuous (with additional hypotheses)

\input{../_definitions/definition_topos_morphism_induced_by_a_morphism_of_small_sites.tex}
\input{../_definitions/definition_flat_functor_between_categories.tex}
...
% \input{../_concepts/proposition_morphism_of_sites_to_morphism_of_topoi.tex}

% \input{../_concepts/theorem_morphisms_of_sites_and_topoi_equivalence.tex}

% \input{../_concepts/proposition_morphism_of_topoi_from_functor.tex}
...
\input{../_definitions/definition_ipc_property_of_a_category_that_admits_small_products_and_small_filtered_limits.tex}

\input{../_concepts/proposition_IPC_property_of_categories_that_admit_small_products_and_small_filtered_limits_is_preserved_by_small_products.tex}

\input{../_concepts/proposition_categories_of_sets_functors_from_small_categories_to_sets_and_modules_satisfy_ipc_property.tex}

\begin{proposition}[{\cite[Proposition 3.1.11(iii)]{kashiwara_schapira_cs}}]
    \TODO{think about whether local smallness can be removed}
    Let $\calA$ be a \CrefAndHyperrefIfExist{lemma:category_of_presheaves_on_a_small_category_of_locally_small_value_is_locally_small}{locally small} \CrefAndHyperrefIfExist{definition:category}{category} that admits \CrefAndHyperrefIfExist{definition:small_and_finite_limits_and_colimits_in_a_category}{small} \CrefAndHyperrefIfExist{definition:product_and_coproduct_of_objects_in_a_category}{products} and small \CrefAndHyperrefIfExist{definition:projective_and_inductive_limits_in_categories}{filtered limits}.

    Assume that there exist a set $I$ and a functor $\lambda: \calA \to \Sets^I$ \CrefIfExists{definition:category_of_sets}\CrefIfExists{definition:diagram_in_a_category_indexed_by_a_category} such that 
    \begin{itemize}
        \item $\lambda$ commutes with \CrefAndHyperrefIfExist{definition:small_and_finite_limits_and_colimits_in_a_category}{small} \CrefAndHyperrefIfExist{definition:product_and_coproduct_of_objects_in_a_category}{products}
        \item $\lambda$ commutes with small \CrefAndHyperrefIfExist{definition:projective_and_inductive_limits_in_categories}{filtered inductive limits}
        \item $\lambda$ is conservative \TODO{conservative}
    \end{itemize}
    Then $\calA$ satisfies the IPC-property
\end{proposition}


The following expresses the idea that sheafification functors exist for small enough sites when the value category is nice enough.
...
\input{../_definitions/definition_Kan_fibration_between_simplicial_sets.tex}
\input{../_definitions/definition_trivial_Kan_fibration_between_simplicial_sets.tex}
\input{../_concepts/proposition_simplicial_set_is_quasi_category_iff_maps_from_Delta_2_to_maps_from_Lambda_1_2_is_a_trivial_kan_fibration.tex}
...
\input{../_concepts/lemma_0_simplex_is_terminal_object_in_category_of_simplicial_sets.tex}

\input{../_definitions/definition_empty_simplicial_set.tex}

\input{../_concepts/lemma_empty_simplicial_set_is_initial_object_in_category_of_simplicial_sets.tex}


\input{../_concepts/lemma_simplicial_set_is_kan_complex_iff_simplicial_set_map_to_final_simplicial_set_is_a_kan_fibration.tex}
...
\section{Limits and colimits in \texorpdfstring{$\infty$}{infty}-categories}

\input{../_definitions/definition_join_of_simplicial_sets.tex}
\input{../_concepts/proposition_existence_of_overcategory_simplicial_set_S_slash_p_with_universal_property.tex}
\input{../_definitions/definition_overcategory_and_undercategory_of_an_infty_category_with_respect_to_a_map_from_a_simplicial_set.tex}
\input{../_definitions/definition_limit_and_colimit_for_a_map_from_a_simplicial_set_to_an_infty_category.tex}
\input{../_definitions/definition_terminal_object_in_an_infty_category.tex}
\input{../_definitions/definition_pullback_and_pushforward_of_maps_to_and_from_objects_in_an_infty_category.tex}

\section{Algebraic objects in \texorpdfstring{$\infty$}{infty}-categories}


\input{../_definitions/definition_simplicial_object_in_an_infty_category.tex}
\input{../_definitions/definition_category_object_in_an_infty_category.tex}
\input{../_definitions/definition_semigroup_object_in_an_infty_category.tex}
\input{../_definitions/definition_monoid_object_in_an_infty_category.tex}
\input{../_definitions/definition_group_object_in_an_infty_category.tex}
...
\input{../_definitions/definition_strict_2_category.tex}

\input{../_definitions/definition_bicategory.tex}

\input{../_concepts/lemma_all_strict_2_categories_are_weak_2_categories.tex}
\begin{proof}
    In a strict $2$-category, the strict associativity and the strict unit laws in particular yield associators and left/right unitors.
\end{proof}
...
\input{../_definitions/definition_2_category_of_categories_relative_to_a_grothendieck_universe.tex}



\input{../_definitions/definition_pseudofunctor_between_weak_2_categories.tex}


\input{../_concepts/theorem_coherence_conditions_for_pseudofunctor_between_weak_2_categories_to_be_well_defined.tex}



\input{../_definitions/definition_pseudonatural_transformation_between_pseudofunctors_between_weak_2_categories.tex}

\input{../_definitions/definition_modification_between_pseudonatural_transformations_between_pseudofunctors_between_weak_2_caetgories.tex}

\input{../_definitions/definition_biequivalence_between_weak_2_categories.tex}

\input{../_concepts/theorem_pseudofunctor_between_weak_2_categories_is_a_biequivalence_if_and_only_if_it_is_locally_an_equivalence_and_is_biessentially_surjective_on_objects.tex}
...
\input{../_concepts/theorem_weak_2_categories_are_biequivalent_to_strict_2_categories.tex}
...
In a strict $n$-category, the associativity and unit axioms are specified ``strictly'' ---- i.e. they are expressed in terms of literal equalities as opposed to looser isomorphisms.
...
\subsection{Ring spectra and algebra objects in stable infinity-categories}

This subsection collects definitions of ring spectrum structures, both the classical homotopy-theoretic notion (monoids in the stable homotopy category) and the highly structured $\infty$-categorical notion ($\mathbb{E}_k$-rings).
...
\input{../_definitions/definition_zermelo_fraenkel_set_theory.tex}
...
\input{../_definitions/definition_transitive_set.tex}

\begin{proposition}
For any set $x$, the following conditions are equivalent:
\begin{enumerate}
    \item $x$ is a \CrefAndHyperrefIfExist{definition:transitive_set}{transitive set}.
    \item $\cup x \subseteq x$.
    \item $x \subseteq \mathcal{P}(x)$, where \hl{$\mathcal{P}(x)$} denotes the power set of $x$.
\end{enumerate}
\end{proposition}
\input{../_definitions/definition_transitive_closure_of_a_set.tex}

\input{../_definitions/definition_ordinal_number.tex}
\begin{theorem}
An \CrefAndHyperrefIfExist{definition:ordinal_number}{ordinal} is a \CrefAndHyperrefIfExist{definition:transitive_set}{transitive set} that is strictly well-ordered by the membership relation $\in$. \TODO{strictly well-ordered}
\end{theorem}

\input{../_definitions/definition_hereditarily_finite_sets.tex}

\input{../_concepts/proposition_collection_of_hereditarily_finite_sets_is_a_set.tex}

\begin{theorem}
A set $x$ is \CrefAndHyperrefIfExist{definition:hereditarily_finite_sets}{hereditarily finite} if and only if $x$ is finite and every element $y \in x$ is also a hereditarily finite set.
\end{theorem}

\begin{proposition}
The structure $(V_\omega, \in)$ satisfies all the axioms of \CrefAndHyperrefIfExist{definition:zermelo_fraenkel_set_theory}{Zermelo-Fraenkel set theory} excluding the Axiom of Infinity.
\end{proposition}

\input{../_concepts/theorem_set_of_all_hereditarily_finite_sets_is_a_grothendieck_universe.tex}
