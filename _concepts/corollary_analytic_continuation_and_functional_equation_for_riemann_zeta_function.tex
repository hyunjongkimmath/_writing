...
The \CrefAndHyperrefIfExist{definition:riemann_zeta_function}{Riemann zeta function} $\zeta(s)$ admits a \CrefAndHyperrefIfExist{definition:meromorphic_continuation_of_a_holomorphic_function_on_an_open_subset_of_the_complex_plane}{meromorphic continuation} to $\bbC$ with a single simple pole at $s = 1$ of residue $1$.

Define the \hldef{completed Riemann zeta function}
$$
\boxed{ \xi(s) = \frac{1}{2} s (s - 1) \pi^{-s/2} \Gamma\!\left( \frac{s}{2} \right) \zeta(s). }
$$
Then $\xi(s)$ is an \CrefAndHyperrefIfExist{definition:entire_function_on_complex_space}{entire function} satisfying the symmetric functional equation
$$
\boxed{ \xi(s) = \xi(1 - s). }
$$
