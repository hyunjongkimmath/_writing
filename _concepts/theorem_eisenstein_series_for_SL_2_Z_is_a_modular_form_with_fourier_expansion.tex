...
\TODO{create the definition of $\sigma_k$ as its own file and reference it}
where $\zeta(s)$ is the \CrefAndHyperrefIfExist{definition:riemann_zeta_function}{Riemann zeta function} and $\sigma_{k-1}(n) = \sum_{d|n} d^{k-1}$ is the \hldef{sum of $(k-1)$-st powers of divisors function}.
...
\begin{enumerate}
  ...
  By \Cref{definition:series_indexed_by_a_countable_set}, the double series $\sum_{(m,n) \neq (0,0)} (m^2 + n^2)^{-k/2}$ is defined by the supremum of its finite partial sums. The function $(x,y) \mapsto (x^2+y^2)^{-k/2}$ is nonincreasing in each variable separately on $[0,\infty)^2$. By \Cref{theorem:comparison_of_double_series_to_double_integrals_of_nonnegative_functions} with $m_0 = n_0 = 1$, we have
	  ...
	  \CrefIfExists{definition:lebesgue_integral_over_a_measurable_subset_of_Rn}
	  ...
	  \end{align*}
	  which is finite for $k > 2$ because $\int_1^\infty r^{1-k}\,dr = 1/(k-2)$. Thus the double series converges. By the \CrefAndHyperrefIfExist{theorem:weierstrass_m_test_for_uniform_convergence_of_function_series}{Weierstrass $M$-test}, the series defining $G_k(z)$ \CrefAndHyperrefIfExist{definition:absolute_convergence_of_a_series_in_module_over_a_commutative_ring_with_an_absolute_value_valued_in_an_ordered_semiring}{converges absolutely} and \CrefAndHyperrefIfExist{definition:uniform_convergence_of_a_sequence_of_functions_from_a_set_to_an_extended_metric_space_valued_in_ordered_commutative_monoid_with_adjoined_infinity}{uniformly} on $K$.
