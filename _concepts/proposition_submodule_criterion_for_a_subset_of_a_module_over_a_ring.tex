...
\end{proposition}
\begin{proof}
    If $N$ is an $R$-$S$-submodule of $M$, then the listed conditions clearly hold by definition. Conversely, suppose that the listed conditions hold. We show that $N$ is indeed an $R$-$S$-submodule of $M$. To show that $N$ is a subgroup of $M$, it then suffices to show that $N$ is closed under additive inverses\CrefIfExists{proposition:subgroup_criterion_for_a_subset_of_a_group}. This is true by the closure under left multiplication by $R$ because $(-1) \cdot n = -n$ \TODO{show that minus one times module element is additive inverse}. Since we are already assuming closure under multiplciation by elements of $R$ and by elements of $S$, we have that $N$ is an $R$-$S$-submodule of $M$ as desired. 
\end{proof}