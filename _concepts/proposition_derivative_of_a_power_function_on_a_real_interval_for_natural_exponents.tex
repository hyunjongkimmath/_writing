...
\begin{proof}
    ...
    Assume the statement holds for some $n \ge 1$, i.e., the function $g(x) = x^n$ is differentiable on $I$ with $g'(a) = n a^{n-1}$ for all $a \in I$. Consider the function $f(x) = x^{n+1}$. We can write $f(x) = x \cdot x^n = x \cdot g(x)$. By the \CrefAndHyperrefIfExist{proposition:sum_rule_product_rule_quotient_rule_chain_rule_for_real_derivatives_of_real_valued_functions_on_real_intervals}{product rule}, $f$ is differentiable at each $a \in I$ and
    \begin{align*}
