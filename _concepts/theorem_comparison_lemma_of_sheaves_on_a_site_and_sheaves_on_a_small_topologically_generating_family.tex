\begin{theorem} [Comparison Lemma, cf. {{\cite[Expos\'e III, Th\'eor\`eme 4.1]{SGA4_I}}}] \label{theorem:comparison_lemma_of_sheaves_on_a_site_and_sheaves_on_a_small_topologically_generating_family}
    ...
    $$u^s: \Sh(\calD, K; \calA) \to \Sh(\calC, J; \calA)$$
...
\end{theorem}
\begin{proof}
    Note that the direct image indeed is a functor between the categories of sheaves by \Cref{proposition:equivalent_conditions_for_continuous_functor_of_sites} because $u: \calC \to \calD$ is a \CrefAndHyperrefIfExist{definition:continuous_functor_of_sites}{continuous functor} between the sites by \Cref{proposition:functor_from_induced_site_to_site_is_continuous_functor}.  
    \TODO{talk about the right kan extension --- in particular, how does it relate to the left Kan extension and to the direct image/inverse image of a functor?}
    \TODO{continue}
