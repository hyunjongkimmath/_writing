...
\input{../_definitions/definition_vector_valued_lebesgue_integral_into_normed_vector_space.tex}
...
\begin{notation}
    Given a \CrefAndHyperrefIfExist{definition:locally_compact_group}{locally compact group} $G$, fix a left Haar measure on $G$; integration with respect to this Haar measure would often be denoted by \hl{$\int_G dg$}
\end{notation}

\begin{definition}
    Let \( G \) be a \CrefAndHyperrefIfExist{definition:locally_compact_group}{locally compact group} with a fixed left Haar measure \( \mu \). For functions $f_1, f_2: G \to \bbC$, their \hldef{convolution} \( f_1 * f_2 \in L^1(G) \) is defined by
...
\begin{lemma}
    Let $G$ be a \CrefAndHyperrefIfExist{definition:locally_compact_group}{locally compact group}. 
    \begin{enumerate}
...
\begin{notation}\label{notation:bilinear_pairing_notation_for_evaluation_of_a_character}
    Let \( G \) be a \CrefAndHyperrefIfExist{definition:locally_compact_group}{locally compact abelian group}. Given $g \in G$ and \CrefAndHyperrefIfExist{definition:pontryagin_dual_of_a_locally_compact_abelian_group}{$\chi \in \widehat{G}$}, write \hl{$\langle g, \chi \rangle$} for $\chi(g) \in \bbC$. 
\end{notation}
...
\begin{notation} \label{notation:characters_of_the_points_of_an_algebraic_group_over_a_finite_field}
    Given an algebraic group $G$ over a finite field $\bbF_q$, and when a prime $\ell \neq \Char \bbF_q$ is understood in context, let \hl{$\widehat{G}(\Fqn)$} denote the set of \CrefAndHyperrefIfExist{definition:quasi_character_of_a_locally_compact_hausdorff_group}{(unitary) characters} $\chi: G \to \Qellbar^\times$. Let \hl{$\widehat{G}$} denote the union
...
\begin{itemize}
    ...
    where $M_{\Fqn}$ is the pullback of $M$ under the base change map $G_{\Fqn} \to G$ and \CrefAndHyperrefIfExist{definition:kummer_sheaf_associated_to_a_character_on_points_of_a_connected_commutative_algebraic_group_over_a_finite_field}{$\calL_{\chi}$} is the \CrefAndHyperrefIfExist{definition:kummer_sheaf_associated_to_a_character_on_points_of_a_connected_commutative_algebraic_group_over_a_finite_field}{Kummer sheaf associated to $\chi$}.
...
\begin{enumerate}
    \item $S(M, \chi) = S(M_\chi, 1_n)$ for any \CrefAndHyperrefIfExist{notation:characters_of_the_points_of_an_algebraic_group_over_a_finite_field}{$\chi \in \widehat{G}(\Fqn)$}. 
    ...
    \item $S(M *_! N, \chi) = S(M, \chi) \cdot S(N,\chi)$ for any \CrefAndHyperrefIfExist{notation:characters_of_the_points_of_an_algebraic_group_over_a_finite_field}{$\chi \in \widehat{G}(\Fqn)$}. 
...
\begin{convention}
    ...
    \begin{enumerate}
        \item the $L^p$-space on a measure space is assumed to be made into a metric space by equipping it with the \CrefAndHyperrefIfExist{definition:extended_metric_induced_by_an_extended_norm_on_a_vector_space_over_a_field_with_absolute_value}{metric induced by} the \CrefAndHyperrefIfExist{definition:L_p_norm_on_a_measure_space}{$L^p$-norm}.  
...
Any element in the $L^p$-space $L^p(X, \mu)$ of a general measure space can be $L^p$-approximated by simple functions with finite measure support. We formulate this idea in terms of \emph{density} of subspaces of $L^p(X, \mu)$ with respect to the \CrefAndHyperrefIfExist{definition:topology_induced_by_a_norm_on_a_vector_space_over_a_field_with_absolute_value}{topology induced by} the \CrefAndHyperrefIfExist{definition:L_p_norm_on_a_measure_space}{$L^p$-norm}
...
\input{../_definitions/definition_banach_space_over_a_commutative_ring_with_an_absolute_value_valued_in_an_ordered_semiring.tex}
